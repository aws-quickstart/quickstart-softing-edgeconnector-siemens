...
[start=3]
. In the *Connection Settings* tab, the default value of the *Connection Name* field is *S7-Sim*, but you can provide a new name, if needed. Keep the default values in the other fields. 
...
* If an Amazon EC2 instance fails and another one is created by the Auto Scaling group, the original AWS IoT SiteWise gateway resource is lost, which means that the CloudFormation template can't delete it when the stack is deleted. To solve this issue, the Amazon EC2 *UserData* script creates the resource, but it needs to be deleted manually.
